...
class EntityXMLWriter extends WorkflowComponentWithSlot {
	override invoke(IWorkflowContext ctx) {
		val model = ctx.get(newEntitiesSlot) as ArrayList<Entity>
		val s = new StringBuilder()
		
		for(e : model) {
			switch e {
				IfcLocalPlacement: {
					s.append("  <_1:IfcLocalPlacement" + (if (!e.id.nullOrEmpty) { " id=\"" + e.id + "\"" } else { "" }) + ">\n")
				...
				IfcAxis2Placement3D: {
					s.append("  <_1:IfcAxis2Placement3D" + (if (!e.id.nullOrEmpty) { " id=\"" + e.id + "\"" } else { "" }) + ">\n")
				...
				IfcCartesianPoint: {
					s.append("  <_1:IfcCartesianPoint" + (if (!e.id.nullOrEmpty) { " id=\"" + e.id + "\"" } else { "" }) + ">\n")
				...
				IfcDirection: {
					s.append("  <_1:IfcDirection" + (if (!e.id.nullOrEmpty) { " id=\"" + e.id + "\"" }) + ">\n")
				...
				IfcProduct: {
					s.append("  <_1:IfcOpeningElement" + (if (!e.id.nullOrEmpty) { " id=\"" + e.id + "\"" } else { "" }) + ">\n")
					s.append(if (!e.globalId.nullOrEmpty) "    <_1:GlobalId>" + e.globalId + "</_1:GlobalId>\n" else "") 
	  				...
	  				s.append(if (!e.name.nullOrEmpty) "    <_1:Name>" + e.name + "</_1:Name>\n" else "")
	  				s.append(if (!e.description.nullOrEmpty) "    <_1:Description>" + e.description + "</_1:Description>\n" else "")
		...
		ctx.put(newEntitiesXMLSlot, s.toString)
	}
}